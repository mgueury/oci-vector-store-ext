...
resource "oci_identity_policy" "starter_search_policy" {
    ...
    statements = [
        ...
        "allow any-user to manage ai-service-document-family  in compartment id ${local.lz_serv_cmp_ocid} where request.principal.id='${oci_core_instance.starter_compute.id}'",
